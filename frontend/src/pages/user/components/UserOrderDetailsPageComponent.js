  ...
  useEffect(() => {
    getOrder(id)
      .then((data) => {
        ...
        if (data.isPaid) {
          setOrderButtonMessage("Your service is completed");
          setButtonDisabled(true);
        } else {
          if (data.paymentMethod === "pp") {
            setOrderButtonMessage("Pay for your service");
          } else if (data.paymentMethod === "cod") {
            setButtonDisabled(true);
            setOrderButtonMessage("Wait for your worker. You pay on delivery");
          }
        }
      })
      .catch((err) => console.log(err));
  ...
  const orderHandler = () => {
    ...
    if (paymentMethod === "pp") {
      setOrderButtonMessage(
        "To pay for your service click one of the buttons below"
      );
      if (!isPaid) {
        loadPayPalScript(cartSubtotal, cartItems, id, updateStateAfterOrder)
      }
    } else {
      setOrderButtonMessage("Your service was booked. Thank you");
    }
  };
      ...
      <Row className="mt-4">
        ...
        <Col md={4}>
          <ListGroup>
            ...
            <ListGroup.Item>
              Services price (after tax):{" "}
              <span className="fw-bold">₹{cartSubtotal}</span>
            </ListGroup.Item>
            <ListGroup.Item>
              Convenience fee: <span className="fw-bold">included</span>
            </ListGroup.Item>
            <ListGroup.Item>
